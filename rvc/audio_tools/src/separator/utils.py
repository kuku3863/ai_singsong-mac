...
try:
    import gradio as gr
    _DefaultProgress = gr.Progress
except (ImportError, AttributeError):
    _DefaultProgress = None
...
def get_model_from_config(model_type, config_path):
    ...
    if model_type == "mdx23c":
        ...
        from .models.bs_roformer.bs_roformer import MelBandRoformer
...
def demix_track(config, model, mix, device, progress=None, progress_desc=""):
    if progress is None:
        if _DefaultProgress is not None:
    ...
    windowingArray = _getWindowingArray(C, fade_size).to(device)
    ...
    with torch.cuda.amp.autocast(enabled=config.training.use_amp):
        with torch.inference_mode():
            ...
            while i < mix.shape[1]:
                ...
                if len(batch_data) >= batch_size or (i >= mix.shape[1]):
                    ...
                    if progress_bar is not None:
                        progress_bar.update(min(step, mix.shape[1] - i + step))
            ...
            if length_init > 2 * original_border and (original_border > 0):
                estimated_sources = estimated_sources[..., original_border:-original_border]
