...
def load_audio_tensor(audio_path):
    try:
        audio, sr = soundfile.read(audio_path, dtype="float32", always_2d=True)
...
class Svc(object):
    def __init__(self, net_g_path, config_path,
        ...
        if device is None:
            # Check for best available device: cuda -> mps -> cpu
        ...
        if self.shallow_diffusion or self.only_diffusion:
            if os.path.exists(diffusion_model_path) and os.path.exists(diffusion_config_path):
    ...
    def get_unit_f0(self, wav, tran, cluster_infer_ratio, speaker, f0_filter ,f0_predictor,cr_threshold=0.05):
        ...
        if cluster_infer_ratio != 0 and self.cluster_model is not None:
            if self.feature_retrieval:
    ...
    def infer(self, speaker, tran, raw_path,
        ...
        wav, sr = load_audio_tensor(raw_path)
        if sr != self.target_sample:
            wav = torchaudio.transforms.Resample(sr, self.target_sample)(wav)
        wav = wav.mean(0).cpu().numpy()
    ...
    def slice_inference(self,
        ...
        try:
            ...
            for (slice_tag, data) in audio_data:
                if slice_tag:
                    continue
                max_seg_sec = max(max_seg_sec, len(data) / float(audio_sr))
        except Exception:
            max_seg_sec = 0.0

        if clip_seconds == 0 and max_seg_sec >= 30.0:
            clip_seconds = 15
            if lg_num == 0:
                lg_num = 0.2
        ...
        global_frame = 0
        audio = []
        for (slice_tag, data) in tqdm(audio_data, desc="Inferencing Slices"):
...
class RealTimeVC:
    ...
    def process(self, svc_model, speaker_id, f_pitch_change, input_wav_path,
        ...
        try:
            ...
            raise ImportError("Real-time inference requires 'scikit-maad' package. Please install it with: pip install scikit-maad")

        audio, sr = load_audio_tensor(input_wav_path)
        audio = audio.cpu().numpy()[0]
        temp_wav = io.BytesIO()
        if self.last_chunk is None:
            input_wav_path.seek(0)

            audio, _, _ = svc_model.infer(speaker_id, f_pitch_change, input_wav_path,
                                        cluster_infer_ratio=cluster_infer_ratio,
                                        auto_predict_f0=auto_predict_f0,
                                        noice_scale=noice_scale,
                                        f0_filter=f0_filter)
            ...
            audio, _, _ = svc_model.infer(speaker_id, f_pitch_change, temp_wav,
                                        cluster_infer_ratio=cluster_infer_ratio,
                                        auto_predict_f0=auto_predict_f0,
                                        noice_scale=noice_scale,
                                        f0_filter=f0_filter)
