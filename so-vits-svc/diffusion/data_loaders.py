...
class AudioDataset(Dataset):
    def __init__(
        ...
        self.paths = _read_lines_any_encoding(filelists)
        valid_paths = []
        for name_ext in tqdm(self.paths, total=len(self.paths)):
            try:
                ...
                print(f"[WARN] skip bad sample: {name_ext} ({e})")
                continue
                        
            try:
                # Use Path to robustly handle separators and extract parent folder name
                spk_name = Path(name_ext).parent.name
            except Exception:
                # Fallback to split if Path fails (unlikely)
                spk_name = name_ext.split("/")[-2]

            if spk is not None and len(spk) > 0:
                if spk_name not in spk:
                    # Skip samples from speakers not in the config
                    continue
                spk_id = spk[spk_name]
            else:
                spk_id = 0
            ...
            if load_all_data:
                ...
                try:
                    ...
                    print(f"[WARN] skip bad cached sample: {name_ext} ({e})")
                    continue
                ...
                try:
                    path_augmel = name_ext + ".aug_mel.npy"
                    _, keyshift = np.load(path_augmel, allow_pickle=True)
                    self.pitch_aug_dict[name_ext] = keyshift
                except Exception as e:
                    print(f"[WARN] skip bad sample: {name_ext} ({e})")
                    continue
            ...
            valid_paths.append(name_ext)
        self.paths = valid_paths
        print(f" [INFO] Dataset loaded. Kept {len(self.paths)} samples for speakers: {list(spk.keys()) if spk else 'All'}")
