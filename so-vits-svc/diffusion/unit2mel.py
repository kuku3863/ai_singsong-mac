...
def load_model_vocoder(
    ...
    print(' [Debug] Start torch.load (CPU)')
    ckpt = torch.load(model_path, map_location='cpu')
    print(' [Debug] Finished torch.load (CPU)')

    # Check for spk_embed in checkpoint to auto-adjust n_spk
    has_spk_embed = "spk_embed.weight" in ckpt['model']
    actual_n_spk = args.model.n_spk
    if not has_spk_embed and actual_n_spk > 1:
        print(f" [Warning] Checkpoint has no speaker embedding, but config says n_spk={actual_n_spk}. Forcing n_spk=1 to avoid noise.")
        actual_n_spk = 1
    ...
    print(' [Debug] Start load_state_dict (strict=False)')
    model.load_state_dict(ckpt['model'], strict=False)
    print(' [Debug] Finished load_state_dict')
    
    print(f' [Debug] Start model.to({device})')
    model.to(device)
    print(f' [Debug] Finished model.to({device})')
    
    model.eval()
    print(f' [Success] Loaded diffusion model, sampler is {args.infer.method}, speedup: {args.infer.speedup} ')
