...
warnings.filterwarnings("ignore", message=".*The value passed into gr.Dropdown().*")
...
def load_model_func(ckpt_name,cluster_name,config_name,enhance,diff_model_name,diff_config_name,only_diffusion,use_spk_mix,using_device,method,speedup,cl_num,vocoder_name):
    ...
    try:
        ...
        if diff_model_name != "no_diff":
            print(f"📄 正在读取扩散配置文件: {diff_config_path}")
        ...
        import traceback
        err_msg = f"❌ 加载模型时发生内部错误: {str(e)}\n{traceback.format_exc()}"
        print(err_msg)
        return err_msg, gr.update(choices=[], value=""), 0, None
...
def load_options():
    ...
    cluster_list = ["no_clu"] + get_file_options(ckpt_read_dir, ".index") + get_file_options(ckpt_read_dir, ".pkl")
    ...
    if os.path.exists(external_dir):
        ext_ckpt = [file for file in get_file_options(external_dir, ".pth") if not file.startswith("D_") or file == "G_0.pth"]
        ext_config = get_file_options(external_dir, ".json")
        ext_cluster = get_file_options(external_dir, ".index") + get_file_options(external_dir, ".pkl")
...
with app:
    ...
    with gr.Tabs():
        with gr.TabItem("推理") as inference_tab:
            ...
            """)
            with gr.Row():
                choice_ckpt = gr.Dropdown(label="模型选择", choices=ckpt_list, value="no_model", allow_custom_value=True)
            ...
            with gr.Row():
                diffusion_method = gr.Dropdown(label="扩散模型采样器", choices=["dpm-solver++","dpm-solver","pndm","ddim","unipc"], value="dpm-solver++", allow_custom_value=True)
                diffusion_speedup = gr.Number(label="扩散加速倍数，默认为10倍", value=10)
            using_device = gr.Dropdown(label="推理设备，默认为自动选择", choices=["Auto",*cuda.keys(),"cpu"], value="Auto", allow_custom_value=True)
            with gr.Row():
            ...
            inference_tab.select(refresh_options,[],[choice_ckpt,config_choice,cluster_choice,diff_choice,diff_config_choice,mode_caption])
        ...
        vc_submit.click(vc_fn, [output_format, sid, vc_input3, vc_transform,auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,f0_predictor,enhancer_adaptive_key,cr_threshold,k_step,use_spk_mix,second_encoding,loudness_envelope_adjustment], [vc_output1, vc_output2], api_name="run_inference")
    ...
    with gr.Tabs():
        ...
        app.queue().launch(server_name="127.0.0.1",server_port=7865,inbrowser=False,quiet=False,show_error=True)
