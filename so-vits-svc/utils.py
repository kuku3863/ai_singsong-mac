...
def plot_data_to_numpy(x, y):
    ...
    try:
        ...
        data = data[:, :, :3]
        # Flatten for reshape below if needed, but actually we can just return it shaped or flatten it to match old behavior logic if needed. 
        # The old code did reshape immediately after.
        # Let's match the old 'flat' output structure before reshape
        data = data.flatten()
...
# Global cache for speech encoders to avoid repeated loading and OOM
_speech_encoder_cache = {}

def get_speech_encoder(speech_encoder,device=None,**kargs):
    global _speech_encoder_cache
    
    # Create a unique cache key based on encoder name and device
    # Use string representation of device for keying
    device_str = str(device) if device else "auto"
    cache_key = (speech_encoder, device_str)
    
    if cache_key in _speech_encoder_cache:
        # print(f" [Debug] Using cached speech encoder: {speech_encoder} on {device_str}")
        return _speech_encoder_cache[cache_key]
    ...
    _speech_encoder_cache[cache_key] = speech_encoder_object
...
def clean_checkpoints(path_to_models='logs/44k/', pattern='G_*.pth', n_ckpts_to_keep=2, sort_by_time=True):
  """Freeing up space by deleting saved ckpts

  Arguments:
  path_to_models    --  Path to the model directory
  pattern           --  Pattern to match (e.g., 'G_*.pth')
  n_ckpts_to_keep   --  Number of ckpts to keep, excluding G_0.pth and D_0.pth
  sort_by_time      --  True -> chronologically delete ckpts
                        False -> lexicographically delete ckpts
  """
  import glob
  ckpts_files = [os.path.basename(f) for f in glob.glob(os.path.join(path_to_models, pattern))]
  ckpts_files = [f for f in ckpts_files if not f.endswith("_0.pth")]
  ...
  x_sorted = sorted(ckpts_files, key=sort_key)
  n_ckpts_to_keep = int(n_ckpts_to_keep)

  if len(x_sorted) <= n_ckpts_to_keep:
      return

  to_del = [os.path.join(path_to_models, fn) for fn in x_sorted[:-n_ckpts_to_keep]]
  def del_info(fn):
