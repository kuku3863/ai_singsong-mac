...
def notify_done(title, message):
    ...
    try:
        ...
        subprocess.Popen(["osascript", "-e", f'display notification "{safe_message}" with title "{safe_title}"'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception:
        pass
...
def list_files(path, extensions=None, recursive=False):
    ...
    for root, _, filenames in os.walk(path):
        ...
        if not recursive:
            break
    return files
def slice_audio_file_v2(*args, **kwargs): pass
def length(*args, **kwargs): return 0,0,0,0,0,[]
from inference.infer_tool import Svc
# from onnx_export import main as onnx_export
def onnx_export(*args, **kwargs): pass
# from sami import SAMIService
class SAMIService: pass
def sami_inference(*args, **kwargs): return None, "SAMI not supported"
def sami_save_fn(*args, **kwargs): return "SAMI not supported"
# from tts_voices import SUPPORTED_LANGUAGES
SUPPORTED_LANGUAGES = ['Auto']
def tts_fn(*args, **kwargs): return "TTS not supported", None
...
if not os.path.exists(default_settings_file):
    default_settings_content = {
        ...
        "webui_settings": {"second_dir": False},
        "sami_settings": {
            "access_key": "",
            "secret_key": "",
            "app_id": ""
    ...
    with open(default_settings_file, "w") as f:
        yaml.safe_dump(default_settings_content, f)
...
def find_file(name, default_dir, external_dir=r"D:\ai_song\RVCSVC-API\assets\weights"):
    if not name or name in ["no_clu", "no_diff", "no_diff_config"]:
        return name
    # 优先检查外部目录
    ext_path = os.path.join(external_dir, name)
    if os.path.exists(ext_path):
        return ext_path
    # 再检查默认目录
    def_path = os.path.join(default_dir, name)
    if os.path.exists(def_path):
        return def_path
    return def_path # 回退到默认路径
...
def load_model_func(ckpt_name,cluster_name,config_name,enhance,diff_model_name,diff_config_name,only_diffusion,use_spk_mix,using_device,method,speedup,cl_num,vocoder_name):
    global model
    external_dir = r"D:\ai_song\RVCSVC-API\assets\weights"
    
    config_path = find_file(config_name, config_read_dir) if not only_diffusion else "configs/config.json"
    diff_config_path = find_file(diff_config_name, config_read_dir) if diff_config_name != "no_diff_config" else "configs/diffusion.yaml"
    ckpt_path = find_file(ckpt_name, ckpt_read_dir)
    cluster_path = find_file(cluster_name, ckpt_read_dir)
    diff_model_path = find_file(diff_model_name, diff_read_dir)
    ...
    if diff_model_name != "no_diff":
        ...
        if not only_diffusion:
            if _content['data'].get('encoder_out_channels') != config["model"].get('ssl_dim'):
                return "扩散模型维度与主模型不匹配，请确保两个模型使用的是同一个编码器", gr.update(choices=[], value=""), 0, None
    ...
    #读取模型各维度并比对，还有小可爱无视提示硬要加载底模的就返回个未初始张量
        ...
        if emb_dim > config["model"]["n_speakers"]:
            return "模型说话人数量与emb维度不匹配", gr.update(choices=[], value=""), 0, None
        if model_dim != config["model"]["ssl_dim"]: 
            return "配置文件与模型不匹配", gr.update(choices=[], value=""), 0, None
    ...
    return (
        output_msg, 
        gr.update(choices=spk_list, value=spk_choice), 
        clip, 
        gr.update(value=100 if k_step_max>100 else k_step_max, minimum=speedup, maximum=k_step_max)
    )
...
def load_options():
    # 默认路径列表
    ckpt_list = [file for file in get_file_options(ckpt_read_dir, ".pth") if not file.startswith("D_") and file != "G_0.pth"]
    config_list = get_file_options(config_read_dir, ".json")
    cluster_list = ["no_clu"] + get_file_options(ckpt_read_dir, ".pt") + get_file_options(ckpt_read_dir, ".pkl")
    diff_list = ["no_diff"] + get_file_options(diff_read_dir, ".pt")
    diff_config_list = ["no_diff_config"] + get_file_options(config_read_dir, ".yaml")

    # 外部路径列表 (D:\ai_song\RVCSVC-API\assets\weights)
    external_dir = r"D:\ai_song\RVCSVC-API\assets\weights"
    if os.path.exists(external_dir):
        ext_ckpt = [file for file in get_file_options(external_dir, ".pth") if not file.startswith("D_") and file != "G_0.pth"]
        ext_config = get_file_options(external_dir, ".json")
        ext_cluster = get_file_options(external_dir, ".pt") + get_file_options(external_dir, ".pkl")
        ext_diff = get_file_options(external_dir, ".pt")
        ext_diff_config = get_file_options(external_dir, ".yaml")
        
        # 合并列表并去重
        ckpt_list = sorted(list(set(ckpt_list + ext_ckpt)))
        config_list = sorted(list(set(config_list + ext_config)))
        cluster_list = ["no_clu"] + sorted(list(set(cluster_list[1:] + ext_cluster))) # 保留 no_clu 在首位
        diff_list = ["no_diff"] + sorted(list(set(diff_list[1:] + ext_diff)))
        diff_config_list = ["no_diff_config"] + sorted(list(set(diff_config_list[1:] + ext_diff_config)))
...
def refresh_options():
    ...
    return (
        ...
        gr.update(choices=config_list),
        gr.update(choices=cluster_list),
        gr.update(choices=diff_list),
        gr.update(choices=diff_config_list),
        gr.update(value=f"""{current_mode}，可在页面底端切换模式""")
    )

def refresh_options2():
    global ckpt_read_dir, config_read_dir, diff_read_dir, current_mode
    ckpt_read_dir = second_dir if second_dir_enable else workdir
    config_read_dir = second_dir if second_dir_enable else config_dir
    diff_read_dir = diff_second_dir if second_dir_enable else diff_workdir
    ...
    return (
        ...
        diff_list,
        diff_config_list,
        f"""{current_mode}，可在页面底端切换模式"""
    )

def source_change(use_microphone):
    if use_microphone:
        return gr.update(sources=["microphone"])
    else:
        return gr.update(sources=["upload"])

def vc_infer(output_format, sid, input_audio, sr, input_audio_path, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num, lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix, second_encoding, loudness_envelope_adjustment, reverb_intensity=0, delay_intensity=0):
    ...
    if reverb_intensity > 0 or delay_intensity > 0:
        ...
        if reverb_intensity > 0:
            reverb_intensity = max(0, min(10, reverb_intensity))
            ...
            board.append(Reverb(room_size=room_size_val, damping=0.6, wet_level=wet_level_val, dry_level=1.0 - wet_level_val, width=1.0))
        
        if delay_intensity > 0:
            delay_intensity = max(0, min(10, delay_intensity))
            try:
                tempo, _ = librosa.beat.beat_track(y=_audio, sr=model.target_sample)
                actual_tempo = tempo if tempo > 0 else 120
            except:
                actual_tempo = 120
            delay_seconds_val = (60.0 / actual_tempo) * 0.5
            delay_mix_val = (delay_intensity / 10.0) * 0.35
            board.append(Delay(delay_seconds=delay_seconds_val, feedback=0.25, mix=delay_mix_val))
        
        _audio = board(_audio, model.target_sample)
...
def vc_fn(output_format, sid, input_audio, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num, lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix, second_encoding, loudness_envelope_adjustment, reverb_intensity, delay_intensity, progress=gr.Progress(track_tqdm=True)):
    global model
    try:
        ...
        output_file_path = vc_infer(output_format, sid, audio, sr, input_audio, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num, lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix, second_encoding, loudness_envelope_adjustment, reverb_intensity, delay_intensity)
        os.remove("temp.wav")
        notify_done("So-VITS-SVC 推理完成", os.path.basename(output_file_path))
...
def vc_batch_fn(output_format, sid, input_audio_files, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num, lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix, second_encoding, loudness_envelope_adjustment, reverb_intensity, delay_intensity, progress=gr.Progress()):
    global model
    try:
        ...
        for file_obj in progress.tqdm(input_audio_files, desc="Inferencing"):
            ...
            output_file_path = vc_infer(output_format, sid, audio, sr, input_audio_path, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num, lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix, second_encoding, loudness_envelope_adjustment, reverb_intensity, delay_intensity)
...
def dataset_preprocess(encoder, f0_predictor, use_diff, vol_aug, skip_loudnorm, num_processes, tiny_enable):
    if precheck_ok:
        ...
        yield accumulated_output, gr.update(value=spk_name)
    else:
        yield "数据集识别未通过，请先识别数据集并确保没有报错信息", None

def train_from_scratch_combined(encoder, f0_predictor, use_diff, vol_aug, skip_loudnorm, num_processes, tiny_enable, gpu_selection):
    # 1. Preprocess
    last_output = ""
    for output, spk in dataset_preprocess(encoder, f0_predictor, use_diff, vol_aug, skip_loudnorm, num_processes, tiny_enable):
        last_output = output
        yield output
    
    # 2. Start Training
    msg = training(gpu_selection, encoder, tiny_enable)
    yield last_output + "\n" + "="*50 + "\n" + msg
...
def clear_output():
    return gr.update(value="Cleared!>_<")
...
app_theme = gr.themes.Default(font=[gr.themes.Font("Microsoft YaHei"), "ui-sans-serif", "system-ui", "sans-serif"])
app = gr.Blocks()
with app:
    gr.Markdown(value="""
    ...
    with gr.Tabs():
        with gr.TabItem("推理") as inference_tab:
            ...
            """)
            with gr.Row():
                choice_ckpt = gr.Radio(label="模型选择", choices=ckpt_list, value=ckpt_list[0] if ckpt_list else "no_model")
                model_branch = gr.Textbox(label="模型编码器", placeholder="请先选择模型", interactive=False)
            with gr.Row():
                config_choice = gr.Dropdown(label="配置文件", choices=config_list, value="no_config", allow_custom_value=True)
                config_info = gr.Textbox(label="配置文件编码器", placeholder="请选择配置文件")
            gr.Markdown(value="""**请检查模型和配置文件的编码器是否匹配**""")
            with gr.Row():
                diff_choice = gr.Dropdown(label="（可选）选择扩散模型", choices=diff_list, value="no_diff", interactive=True, allow_custom_value=True)
                diff_config_choice = gr.Dropdown(label="扩散模型配置文件", choices=diff_config_list, value="no_diff_config", interactive=True, allow_custom_value=True)
            with gr.Row():
                cluster_choice = gr.Dropdown(label="（可选）选择聚类模型/特征检索模型", choices=cluster_list, value="no_clu", allow_custom_value=True)
                vocoder_choice = gr.Dropdown(label="选择声码器", choices=vocoder_options, value="nsf_hifigan_finetuned", allow_custom_value=True)
            refresh = gr.Button("刷新选项")
            refresh2 = gr.Button("刷新选项2", visible=False)
            with gr.Row():
            ...
            with gr.Row():
                ...
                sid = gr.Dropdown(label="So-VITS说话人", value="speaker0", allow_custom_value=True)
            ...
            refresh2.click(refresh_options2,[],[choice_ckpt,config_choice,cluster_choice,diff_choice,diff_config_choice,mode_caption], api_name="refresh_options")
            ...
            with gr.Accordion("高级设置（一般不需要动）", open=False):
                ...
                reverb_intensity = gr.Slider(label="混响强度 (0-10, 0为关闭)", minimum=0, maximum=10, value=0, step=0.1)
                delay_intensity = gr.Slider(label="回声强度 (0-10, 0为关闭)", minimum=0, maximum=10, value=0, step=0.1)
        ...
        loadckpt.click(load_model_func,[choice_ckpt,cluster_choice,config_choice,enhance,diff_choice,diff_config_choice,only_diffusion,use_spk_mix,using_device,diffusion_method,diffusion_speedup,cl_num,vocoder_choice],[model_message, sid, cl_num, k_step], api_name="load_model")
        unload.click(model_empty_cache, [], [sid, model_message], api_name="unload_model")
        use_microphone.change(source_change, [use_microphone], [vc_input3])
        vc_submit.click(vc_fn, [output_format, sid, vc_input3, vc_transform,auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,f0_predictor,enhancer_adaptive_key,cr_threshold,k_step,use_spk_mix,second_encoding,loudness_envelope_adjustment,reverb_intensity,delay_intensity], [vc_output1, vc_output2], api_name="run_inference")
        
        vc_batch_submit.click(vc_batch_fn, [output_format, sid, vc_batch_files, vc_transform,auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,f0_predictor,enhancer_adaptive_key,cr_threshold,k_step,use_spk_mix,second_encoding,loudness_envelope_adjustment,reverb_intensity,delay_intensity], [vc_output1])
        vc_tts_submit.click(tts_fn, [text_input, tts_gender, tts_lang, tts_rate, tts_volume, output_format, sid, vc_transform,auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,f0_predictor,enhancer_adaptive_key,cr_threshold,k_step,use_spk_mix,second_encoding,loudness_envelope_adjustment,reverb_intensity,delay_intensity], [vc_output1, vc_output2])
        #interrupt_button.click(fn=None, inputs=None, outputs=None, cancels=[vc_event])

        with gr.TabItem("极简训练(推荐)"):
            gr.Markdown(value="""### 极简训练模式
            只需两步：1. 识别数据集 -> 2. 点击从头训练（或继续训练）
            所有关键预处理步骤（重采样、生成配置、提取特征）都已自动集成在“从头训练”中。""")
            with gr.Row():
                simple_get_raw_dirs = gr.Button("第一步：识别数据集 (dataset_raw)", variant="primary")
                simple_raw_dirs_list = gr.Textbox(label="识别结果", placeholder="识别结果将显示在这里...")
            
            with gr.Row():
                simple_encoder = gr.Dropdown(label="编码器 (建议 vec768l12)", choices=encoder_list, value="vec768l12", interactive=True)
                simple_f0_predictor = gr.Dropdown(label="f0预测器 (建议 rmvpe)", choices=f0_options, value="rmvpe", interactive=True)
                simple_gpu = gr.Textbox(label="显卡ID (多卡用逗号隔开)", value=gpus, interactive=True)
            
            with gr.Row():
                simple_train_btn = gr.Button("第二步：从头开始一键训练", variant="primary")
                simple_continue_btn = gr.Button("或者：继续上一次训练", variant="secondary")
            
            simple_output = gr.Textbox(label="训练状态/日志输出", max_lines=20)
            
            with gr.Accordion(label="更多高级选项", open=False):
                with gr.Row():
                    simple_use_diff = gr.Checkbox(label="启用浅扩散", value=True)
                    simple_vol_aug = gr.Checkbox(label="启用响度嵌入", value=False)
                    simple_tiny = gr.Checkbox(label="启用TINY模式", value=False)
                with gr.Row():
                    simple_skip_loudnorm = gr.Checkbox(label="跳过响度匹配")
                    simple_num_processes = gr.Slider(label="预处理线程数", minimum=1, maximum=multiprocessing.cpu_count(), value=1, step=1)
        ...
        with gr.TabItem("小工具/实验室特性"):
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("模型压缩工具"):
                    ...
                    """)
                    model_to_compress = gr.Dropdown(label="模型选择", choices=ckpt_list, value="no_model", allow_custom_value=True)
                ...
                with gr.TabItem("模型发布打包/安装"):
                    ...
                    with gr.Tabs():
                        ...
                        with gr.TabItem("打包"):
                            with gr.Row():
                                model_to_pack = gr.Dropdown(label="选择要打包的模型", choices=ckpt_list, value="", allow_custom_value=True)
                                model_config = gr.Dropdown(label="选择要打包的模型配置文件", choices=config_list, value="", interactive=True, allow_custom_value=True)
                                speaker_name = gr.Textbox(label="模型说话人名称", placeholder="该模型的说话人名称，仅限数字字母下划线，如模型中有多说话人，请用逗号分割，例如：spk1,spk2,spk3", value = "")
                            with gr.Row():
                                diff_to_pack = gr.Dropdown(label="（可选）选择要打包的扩散模型", choices=diff_list, value="no_diff", allow_custom_value=True)
                                cluster_to_pack = gr.Dropdown(label="（可选）选择要打包的聚类或特征检索模型", choices=cluster_list, value="no_cluster", allow_custom_value=True)
                ...
                with gr.TabItem("歌曲人声分离"):
                    ...
                    """)
                    with gr.Row():
                        input_audio = gr.Audio(label="上传原始音频", type="filepath")
        ...
        simple_get_raw_dirs.click(load_raw_dirs,[],[simple_raw_dirs_list])
        simple_train_btn.click(train_from_scratch_combined, [simple_encoder, simple_f0_predictor, simple_use_diff, simple_vol_aug, simple_skip_loudnorm, simple_num_processes, simple_tiny, simple_gpu], [simple_output])
        simple_continue_btn.click(continue_training, [simple_gpu, simple_encoder], [simple_output])
    ...
    app.queue().launch(server_name="127.0.0.1",server_port=7865,inbrowser=False,quiet=False, theme=app_theme)
