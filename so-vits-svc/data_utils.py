...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        # Robust spk mapping: handle both dict access and attribute access
        if isinstance(self.spk_map, dict):
            spk_id = self.spk_map.get(spk, 0)
        else:
            # Fallback for HParams object
            try:
                spk_id = getattr(self.spk_map, spk)
            except AttributeError:
                # If key missing, default to 0
                spk_id = 0
                
        spk = torch.LongTensor([spk_id])
        ...
        if abs(c.size(-1) - spec.size(-1)) >= 3:
            print(f"Skipping {filename}: feature length mismatch. c={c.size(-1)}, spec={spec.size(-1)}")
            return None
        if abs(audio_norm.shape[1]-lmin * self.hop_length) >= 3 * self.hop_length:
            print(f"Skipping {filename}: audio length mismatch. audio={audio_norm.shape[1]}, expected={lmin * self.hop_length}")
            return None
    ...
    def __getitem__(self, index):
        if self.all_in_mem:
            ...
            ret = self.get_audio(self.audiopaths[index][0])

        if ret is None:
            return None
        return self.random_slice(*ret)
