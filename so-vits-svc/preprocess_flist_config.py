...
if __name__ == "__main__":
    ...
    parser.add_argument("--speech_export", type=str, nargs="+", help="List of speakers to include in config/filelists")
    parser.add_argument("--config_path", type=str, default="configs/config.json", help="path to config")
    parser.add_argument("--diffusion_config_path", type=str, default="configs/diffusion.yaml", help="path to diffusion config")
    args = parser.parse_args()
    
    config_template =  json.load(open("configs_template/config_tiny_template.json", encoding="utf-8")) if args.tiny else json.load(open("configs_template/config_template.json", encoding="utf-8"))
    
    # Ensure data section exists and points to the correct filelists
    if "data" not in config_template:
    ...
    if args.speech_export:
        speakers = [
            s for s in args.speech_export
            if os.path.isdir(os.path.join(args.source_dir, s))
        ]
    else:
        speakers = [
            s for s in os.listdir(args.source_dir)
            if os.path.isdir(os.path.join(args.source_dir, s))
        ]
        speakers.sort(key=lambda x: x.lower())
    ...
    for speaker in tqdm(speakers):
        ...
        for file_name in os.listdir(os.path.join(args.source_dir, speaker)):
            ...
            if get_wav_duration(file_path) < 0.5:
    ...
    cfg_basename = os.path.basename(args.config_path).lower()
    if cfg_basename == "config.json":
        project_name = "44k"
    else:
        project_name = os.path.splitext(os.path.basename(args.config_path))[0]
    if "env" in d_config_template and isinstance(d_config_template["env"], dict):
        d_config_template["env"]["expdir"] = f"logs/{project_name}/diffusion"
    
    config_template["spk"] = spk_dict
    config_template["model"]["n_speakers"] = spk_id
    model_encoder = "vec768l12-onnx" if args.speech_encoder == "vec768l12" else args.speech_encoder
    config_template["model"]["speech_encoder"] = model_encoder
    ...
    logger.info(f"Writing to {args.config_path}")
    with open(args.config_path, "w", encoding="utf-8") as f:
        json.dump(config_template, f, indent=2, ensure_ascii=False)
    logger.info(f"Writing to {args.diffusion_config_path}")
    du.save_config(args.diffusion_config_path, d_config_template)
