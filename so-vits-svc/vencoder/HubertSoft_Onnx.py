...
class HubertSoft_Onnx(SpeechEncoder):
    def __init__(self, vec_path="pretrain/hubert-soft.onnx", device=None):
        ...
        elif device == 'mps' or device == torch.device("mps"):
        ...
        try:
            ...
            if 'CUDAExecutionProvider' in providers or 'CoreMLExecutionProvider' in providers:
                provider_name = 'CUDA' if 'CUDAExecutionProvider' in providers else 'CoreML'
                print(f"Warning: ONNX {provider_name} initialization failed: {e}. Falling back to CPU.")
                providers = ['CPUExecutionProvider']
                self.model = onnxruntime.InferenceSession(vec_path, providers=providers)
            else:
                raise e
